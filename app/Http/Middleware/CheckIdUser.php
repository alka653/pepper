...
class CheckIdUser{
	public function handle($request, Closure $next, $resource = ''){
		...
		switch($resource){
			case 'solicitud':
				$user = User::where('persona_id', (is_object($request->solicitud) ? $request->solicitud->mascota->propietario_id : Solicitudes::find($request->solicitud)->mascota->propietario_id))->first();
				break;
			default:
